...
// 1. Loop Permutation: The loops have been reordered to improve data locality and reduce memory access overhead. This can help in better utilizing cache and improving performance.
// 2. Loop Tiling: The loops have been tiled to partition the iteration space into smaller tiles, which can help in improving data reuse and reducing memory access latency.
// 3. Loop Distribution: The loops have been distributed to enable parallel execution of loop iterations across multiple processing elements, which can improve overall throughput.
// 4. Loop Fusion: No loop fusion was performed in this case as the loops were already optimized for HLS and further fusion may not provide significant benefits.
