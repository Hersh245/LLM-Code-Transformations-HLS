...
{
    memcpy_wide_bus_read_double_2d_60_256(A_4_0_buf,0,0,(class ap_uint< 256 > *)A,(0 * 8),sizeof(double ) * ((unsigned long )4800L),4800L);
// Existing HLS partition: #pragma HLS array_partition variable=A_4_1_buf cyclic factor = 4 dim=2
    memcpy_wide_bus_read_double_2d_60_256(A_4_1_buf,0,0,(class ap_uint< 256 > *)A,(0 * 8),sizeof(double ) * ((unsigned long )4800L),4800L);
  }
//BLAS PARAMS
//TRANS = 'N'
//UPLO  = 'L'
...
#pragma HLS pipeline
        if (j <= i) {
          C_buf[i][j] *= beta;
          C_buf[i][j] += alpha * A_4_1_buf[i][k] * A_4_0_buf[j][k];
        }
      }
    }
  }
