...
#pragma HLS INTERFACE m_axi port=A offset=slave depth=7500 bundle=merlin_gmem_kernel_doitgen_128_0
  
#pragma HLS INTERFACE m_axi port=C4 offset=slave depth=450 bundle=merlin_gmem_kernel_doitgen_128_1
  
#pragma HLS INTERFACE m_axi port=sum offset=slave depth=4 bundle=merlin_gmem_kernel_doitgen_512_0
...
#pragma ACCEL interface variable=A depth=25,20,30 max_depth=25,20,30
  double C4_5_0_buf[30][30];
  
#pragma HLS array_partition variable=C4_5_0_buf cyclic factor=2 dim=2
  
#pragma HLS array_partition variable=C4_5_0_buf complete dim=1
  double A_5_0_buf[25][20][30];
  
#pragma HLS array_partition variable=A_5_0_buf complete dim=3
  double sum_buf[30];
  
#pragma HLS array_partition variable=sum_buf cyclic factor=8 dim=1
  int r;
  int q;
  int p;
  int s;
{
    memcpy_wide_bus_read_double_3d_20_30_128(A_5_0_buf,0,0,0,(class ap_uint< 128 > *)A,(0 * 8),sizeof(double ) * ((unsigned long )15000L),15000L);
  }
{
    memcpy_wide_bus_read_double_2d_30_128(C4_5_0_buf,0,0,(class ap_uint< 128 > *)C4,(0 * 8),sizeof(double ) * ((unsigned long )900L),900L);
  }
  merlinL5:
  for (p = 0; p < 30; p++) 
...
#pragma HLS unroll
          _sum_tmp_rdc[s] += A_5_0_buf[r][q][s] * C4_5_0_buf[s][p];
        }
{
          int s_1;
          merlinL0:
...
// Existing HLS partition: #pragma HLS array_partition variable=C4_5_0_buf cyclic factor = 2 dim=2
