// Based on the performance estimates provided, we can optimize the code for HLS by applying loop tiling and loop permutation to improve the trip count (TC), accumulated cycles (AC), and cycles per call (CPC).
// 
...
// 1. Loop Tiling: By tiling the loops, we can improve data locality and reduce memory access overhead, which can lead to better performance in terms of TC, AC, and CPC.
// 2. Loop Permutation: Reordering the loops can help optimize the data access patterns and exploit parallelism more effectively, resulting in improved performance metrics.