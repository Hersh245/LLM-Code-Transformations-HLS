...
// 1. Loop Tiling: By tiling the loops, we can improve data locality and reduce cache misses, leading to better performance. This can be seen in the improved accumulated cycles (AC) and cycles per call (CPC) in the performance estimates.
// 2. Loop Permutation: Reordering the loops can help in optimizing the memory access patterns and pipeline utilization, resulting in better performance metrics such as accumulated cycles (AC) and cycles per call (CPC).